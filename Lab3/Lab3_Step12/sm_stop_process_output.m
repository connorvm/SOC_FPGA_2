...
d = squeeze(mp.Avalon_Sink_Data.Data);    % data      Note: the Matlab squeeze() function rooms singleton dimensions, so we can treat the resulting variable simply as an array.
c = squeeze(mp.Avalon_Sink_Channel.Data); % data
v = squeeze(mp.Avalon_Sink_Valid.Data);   % data
left_index = 1;
right_index = 1;
for i=1:length(v)
    if v(i) == 1  % chek if valid, valid is asserted when there is data
